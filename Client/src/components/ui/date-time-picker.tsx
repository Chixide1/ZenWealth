                ...
                MonthCaption: ({ calendarMonth }) => {
                    return (
                        <div className="inline-flex gap-2">
                            ...
                            >
                                <SelectTrigger className="w-fit gap-1 border-none px-2 focus:bg-accent focus:text-accent-foreground focus:ring-neutral-600 shadow-none">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="bg-neutral-700/90 border-neutral-600 text-primary backdrop-blur-sm">
                                    {MONTHS.map((month) => (
                                        <SelectItem key={month.value} value={month.value.toString()} className="hover:bg-background">
                                            {month.label}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            ...
                            >
                                <SelectTrigger className="w-fit gap-1 border-none shadow-none focus:ring-neutral-600 px-2 focus:bg-accent focus:text-accent-foreground">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="bg-neutral-700/90 border-neutral-600 text-primary backdrop-blur-sm">
                                    {YEARS.map((year) => (
                                        <SelectItem key={year.value} value={year.value.toString()} className="hover:bg-background">
                                            {year.label}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
...
const DateTimePicker = React.forwardRef<Partial<DateTimePickerRef>, DateTimePickerProps & {mode: "range", selected: DateRange, onSelect: (range: DateRange | undefined) => void}>(
    (
        ...
        const [displayDate] = React.useState<Date | undefined>(value ?? undefined);
